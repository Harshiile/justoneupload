...
export default function Layout({ children }: { children: ReactNode }) {
  ...
  return !user ? (
    ...
    <div>
      <SidebarProvider>
        ...
        <Sidebar className="w-[10vw] border-secondary">
          ...
          <SidebarContent className="bg-primary text-white flex justify-between overflow-clip">
            <SidebarGroup>
              <SidebarGroupLabel className="text-white mb-3">
                OVERVIEW
              </SidebarGroupLabel>
              <SidebarGroupContent>
                <SidebarMenu>
                ...
                >
                  <SidebarGroup className="border-t border-secondary transition-all duration-200">
                    <SidebarGroupContent>
                      <SidebarMenu>
                        <SidebarMenuItem>
                          <SidebarMenuButton className="hover:bg-primary hover:text-white hover:cursor-pointer active:bg-primary active:text-white">
                            <div
                              className="flex items-center gap-x-2"
                              onClick={(_) => setConfirmLogout(true)}
                            >
      ...
      {/* Child Content */}
      <div
        className={`fixed top-0 left-[10vw] w-[90vw] h-screen grid place-items-center ${
          isMobile && "w-full"
        }`}
      >
        {children}
