...
export async function GET(req: NextRequest) {
  ...
  try {
    const refTkn = await JwtValidate<User>(refreshToken);
    ...
    try {
      ...
      Res.cookies.set(
        "acsTkn",
        JwtGenerate(userData, process.env.ACCESS_TOKEN_EXPIRY!),
        cookieOptions
      );
      return Res;
    } catch (error) {
      JOUError(400, `${process.env.SERVER_ERROR_MESSAGE} - 1003`);
    }
  } catch (error) {
    return JOUError(403, "Unauthorized User - Log Out");
  }
}
