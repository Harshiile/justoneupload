...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (user.role != "editor")
      return JOUError(400, "Youtuber can't joined any workspace");
    ...
    try {
      ...
      console.log("isEditorExist : ", isEditorExist);

      if (isEditorExist) {
        if (!isEditorExist.authorize)
          return JOUError(400, "Your request is not approve yet");
    ...
    await db
      ...
      .catch((err) => {
        return JOUError(400, `${process.env.SERVER_ERROR_MESSAGE} - 1006`);
      });

    // 3. Mail to Youtuber
    try {
      try {
        const workspacMetaData = await fetchWorkspaceMetadata(
          workspaceId,
          null
        );
        if (!workspacMetaData)
          return JOUError(400, "Workspace Params Not Found");
      ...
      return JOUError(400, "Workspace Not Found");
    }
  } catch (error) {
