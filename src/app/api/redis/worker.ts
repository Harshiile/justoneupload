...
const worker = new Worker('video-scheudler-queue', async job => {
    ...
    await uploadVideoOnYoutuber(fileId, workspaceId).catch(_ => { throw new Error(_.message) })
...
const uploadVideoOnYoutuber = async (fileId: string, workspaceId: string) => {
    try {
        const [video] = await db
            ...
            )).catch(_ => { throw new Error("Video Fetching Error") })
        ...
        const videoStream = await getFileFromDrive(fileId).catch(_ => { throw new Error(`${_.message} For Video`) })
        ...
        if (video.thumbnail) {
            ...
            await yt.thumbnails.set({
                ...
                media: {
                    body: await getFileFromDrive(video.thumbnail).catch(_ => { throw new Error(`${_.message} For Thumbnail`) })
                }
            })
                .catch(err => { throw new Error("Thumbnail Uploading Error") })

            // await deleteOnDrive(video.thumbnail).catch(_ => { throw new Error("Deleting Thumbnail From Drive Error") })
        ...
        await deleteOnDrive(fileId).catch(_ => { throw new Error("Deleting Video From Drive Error") })
        ...
        //     .catch(_ => { throw new Error("Delete Video Details In DB Error") })
        ...
        await db.insert(VideoWorkspaceJoinTable).values({
            ...
            .catch(_ => { throw new Error("Inserting Uploaded Video Details in DB") })
...
const deleteOnDrive = async (fileId: string) => {
    await drive.files.delete({ fileId }).catch(_ => { throw new Error() })
}